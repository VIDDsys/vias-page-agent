...
0.11.0 是可靠性与安全性重构版本：外观延续原有黑白灰设计，核心执行链路、任务隔离、取消、幂等、隐私和测试体系已重做。
...
付款、购买、转账、删除、发布、发送消息、填写敏感信息、跨站导航，以及未被用户明确要求的表单提交，会在侧边栏显示操作预览并等待确认。
...
当前测试覆盖 manifest、模型请求适配、任务协议、安全边界、真实 Edge 扩展加载，以及快照/ref、DOM 变化后旧快照拒绝、fill/check 幂等、operationId、repeat 取消、长文本、Shadow DOM 和密码脱敏；完整集成测试还覆盖本地假模型 → 侧边栏 → 后台 Agent → 审批 → 页面操作 → 新快照反馈 → 最终回复。
